...
class SettingsView(ScrollArea):
    ...
    def init_ui(self):
        ...
        self.add_checkbox(self.tr("Always on Top"), config_model.config.general.always_on_top)
        self.add_checkbox(self.tr("Show Status Overlay"), config_model.config.general.overlay_enabled)
        self.add_checkbox(self.tr("Check for updates on startup"), config_model.config.general.check_update_on_startup)
        self.add_dropdown(
        ...
        self.add_text_input("FPS", str(config_model.config.capture.fps))

        self.add_section(self.tr("Input"))
        self.add_text_input(self.tr("Jitter (ms)"), str(config_model.config.input.jitter_ms))
    ...
    def add_checkbox(self, label: str, checked: bool = False) -> None:
        card = CardWidget()
