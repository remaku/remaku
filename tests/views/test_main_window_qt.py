...
class FakePackExplorerView(QWidget):
    def __init__(self, parent=None) -> None:
        super().__init__(parent)
        self.setObjectName("packs")
...
def test_main_window_creates_child_views(monkeypatch, qtbot) -> None:
    ...
    monkeypatch.setattr(main_window, "PackExplorerView", FakePackExplorerView)
    ...
    assert window.pack_explorer_view.objectName() == "packs"
...
def test_main_window_registers_home_and_settings_views(monkeypatch, qtbot) -> None:
    ...
    assert window.stackedWidget.indexOf(window.pack_explorer_view) >= 0
